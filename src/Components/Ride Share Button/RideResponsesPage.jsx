...
export default function RideResponsesPage({ route, dark, onBack }) {
  ...
  useEffect(() => {
    ...
    // Demo cards (rideShareDemoData.js) have negative ids that don't exist
    // in the real DB — GET /api/ride-routes/:id/responses would just 404
    // for them. Read straight from the hardcoded map instead, skipping the
    // fetch (and its loading flicker) entirely.
    if (route.isDemo) {
      setResponses(DEMO_RIDE_RESPONSES[String(route.id)] || []);
      setLoading(false);
      setError("");
      return;
    }
