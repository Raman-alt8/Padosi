...
export default function ServiceListingsAllPage({ listings = [], onUpdate, onDelete, dark }) {
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className={`h-[64px] flex items-center justify-between px-6 sticky top-0 z-10 border-b ${headerBg}`}>
          <button
            onClick={close}
        ...
        <div className="w-full px-6 py-6 flex flex-col gap-5">
          ...
          {/* Sub-label */}
          <p className={`text-[11px] font-bold uppercase tracking-widest ${metaCol}`}>
            {listings.length === 0
              ? "No services posted yet"
              : `${listings.length} service${listings.length !== 1 ? "s" : ""} in your neighbourhood`}
          </p>
          ...
          {listings.length === 0 ? (
            ...
            <div
              ...
              style={{ gridAutoRows: "calc((100vh - 64px - 88px) / 2 - 8px)" }}
            >
